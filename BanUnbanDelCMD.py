...
async def remove_members(chat_id):
    try:
        # Get chat members and remove them
        async for member in bot.get_chat_members(chat_id):
            if not member.user.is_bot:
                try:
                    await bot.ban_chat_member(chat_id=chat_id, user_id=member.user.id)
                    await bot.unban_chat_member(chat_id=chat_id, user_id=member.user.id)
                    logging.info(f"Removed {member.user.id} from chat {chat_id}")
                    username = member.user.username if member.user.username else "NoUsername"
                    log_removed_user(member.user.id, username)
                except Exception as e:
                    logging.error(f"Failed to remove member {member.user.id}: {e}")
    except Exception as e:
